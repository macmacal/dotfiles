...
echo "----------------------------------------------------"
SUDO_CFG_LINE="auth sufficient pam_u2f.so cue origin=pam://${HOST} appid=pam://${HOST}"
...
if [ ! -f "$SUDO_FILE" ]; then
    ...
    if ! grep -Fq "$SUDO_CFG_LINE" $SUDO_FILE; then
        ...
        echo "$SUDO_CFG_LINE"
        read -p "Insert at the top of the file? [Y/n]: " answer
        case "$answer" in
            [Yy]*|"")
...
echo "----------------------------------------------------"
GDM_CFG_LINE="auth sufficient pam_u2f.so cue userverification=1 nouserok origin=pam://${HOST} appid=pam://${HOST}"
...
if [ ! -f "$GDM_FILE" ]; then
    ...
    if ! grep -Fq "${GDM_CFG_LINE}" "${GDM_FILE}"; then
        ...
        case "$answer" in
            [Yy]*|"")
                # Insert line at the top using sudo (preserves comments at the start)
                sudo sed -i "1i$GDM_CFG_LINE" "$GDM_FILE"
...
if [ ! -f "$KDE_FILE" ]; then
    ...
    if ! grep -Fq "$KDE_LINE" "$KDE_FILE"; then
        ...
        echo "$KDE_LINE"
        read -p "Comment out pam_fprintd.so and add U2F line? [Y/n]: " answer
        case "$answer" in
            [Yy]*|"")
